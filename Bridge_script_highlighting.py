...
summarized_data = []
for i, text in enumerate(patient_data):
    summary = summarizer(text, max_length=50, min_length=25, do_sample=False)[0]['summary_text']
    source_info = f" ([Source: Line {i+1}](#line-{i+1}))"
    summarized_data.append(summary + source_info)
...
# Write categorized and summarized patient data to a Markdown file in the same directory as the script
script_dir = os.path.dirname(os.path.abspath(__file__))
output_file_path = os.path.join(script_dir, f"SUMMARY_{os.path.basename(file_path)}.md")
